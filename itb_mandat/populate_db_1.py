...
def generate_data():
    ...
    # --- PENAMBAHAN BAGIAN BARU & PENGHAPUSAN BAGIAN LAMA ---

    # 10. Status Keuangan (BARU)
    sql_statements.append("\n-- 10. STATUS KEUANGAN\n")
    for mhs in mahasiswa_list:
        tagihan = random.choice([12500000, 15000000, 20000000])
        status_bayar = random.choices(['Lunas', 'Belum Lunas'], weights=[90, 10], k=1)[0]
        batas_bayar = f"{tahun_semester}-08-15"
        sql_statements.append(f"INSERT INTO status_keuangan (nim, tahun, semester, tagihan, pembayaran, tanggal_batas_pembayaran) VALUES ({mhs['nim']}, {tahun_semester}, '{nama_semester}', {tagihan}.00, '{status_bayar}', '{batas_bayar}');")

    # DATA BIMBINGAN TELAH DIHAPUS DARI SINI
...
if __name__ == "__main__":
    ...
    nama_file_output = "data_populasi_final.sql"
    with open(nama_file_output, 'w', encoding='utf-8') as f:
        f.write(semua_data_sql)
    print(f"✅ Berhasil! Data populasi final telah dibuat di file '{nama_file_output}'")